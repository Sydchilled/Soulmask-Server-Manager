...
function _doStartServer(id) {
  ...
  // Try to read game version from existing WS.log before starting
  try {
    const logFile = path.join(cfg.serverPath, 'WS', 'Saved', 'Logs', 'WS.log');
    if (fs.existsSync(logFile)) {
      const logContent = fs.readFileSync(logFile, 'utf8').slice(-50000);
      const verMatch = logContent.match(/[Vv]ersion(?:\s+is(?:\s+now)?)?\s*[:\s]\s*(\d+\.\d+[\.\d]*)/);
      if (verMatch) { inst.gameVersion = verMatch[1]; }
    }
  } catch {}
  ...
  try {
    ...
    const parseLine = (line, type) => {
      ...
      // Capture game version string from startup log
      // Looks like: "Version is now 1.0.1" or "LogInit: Game Version: 1.0.1" or "version 1.0.1"
      const verMatch = line.match(/[Vv]ersion(?:\s+is(?:\s+now)?)?\s*[:\s]\s*(\d+\.\d+[\.\d]*)/);
      if (verMatch && !instances[id].gameVersion) {
        instances[id].gameVersion = verMatch[1];
        addLog(id, 'save', `\u{1f3ae} Game version: ${verMatch[1]}`);
      }
...
function getStatus(id) {
  ...
  return {
    ...
    mods: cfg?.mods || [],
    updating: !!(instances[id]?.updating),
    buildId: instances[id]?.buildId || getInstalledBuildId(cfg?.serverPath) || null,
    gameVersion: (() => {
      // Return cached version if we have it
      if (instances[id]?.gameVersion) return instances[id].gameVersion;
      // Otherwise scan WS.log — works even when server is offline
      try {
        const logFile = path.join(cfg?.serverPath || '', 'WS', 'Saved', 'Logs', 'WS.log');
        if (fs.existsSync(logFile)) {
          const tail = fs.readFileSync(logFile, 'utf8').slice(-80000);
          const m = tail.match(/[Vv]ersion(?:\s+is(?:\s+now)?)?\s*[:\s]\s*(\d+\.\d+[\.\d]*)/);
          if (m) {
            if (!instances[id]) instances[id] = {};
            instances[id].gameVersion = m[1]; // cache it
            return m[1];
          }
        }
      } catch {}
...
function getInstalledBuildId(serverPath) {
  try {
    // Dedicated server app IDs: 3017310 (Windows), 3017300 (Linux)
    // Also check 2646460 (game client) as fallback
    const ids = [STEAM_APP_ID[os.platform()] || '3017310', '3017310', '3017300', '2646460'];
    const candidates = [];
    ids.forEach(id => {
      candidates.push(path.join(serverPath, 'steamapps', `appmanifest_${id}.acf`));
      candidates.push(path.join(serverPath, '..', `appmanifest_${id}.acf`));
      candidates.push(path.join(serverPath, '..', '..', `appmanifest_${id}.acf`));
      candidates.push(path.join(serverPath, '..', '..', '..', `appmanifest_${id}.acf`));
    });
    for (const mp of candidates) {
      if (fs.existsSync(mp)) {
        const m = fs.readFileSync(mp, 'utf8').match(/"buildid"\s+"(\d+)"/);
        if (m) return m[1];
      }
    }
  } catch {}
...
function checkForUpdates(id, callback) {
  const cfg = getServer(id) || loadServers()[0];
  const appId = STEAM_APP_ID[os.platform()] || STEAM_APP_ID.linux;
  ...
  if (!instances[id]) instances[id] = {};
  instances[id].updating = true;

  const currentBuildId = getInstalledBuildId(cfg.serverPath);
  if (currentBuildId) {
    addLog(id, 'info', '📦 Current installed build ID: ' + currentBuildId);
    instances[id].buildId = currentBuildId;
  }

  addLog(id, 'info', '🔄 Running SteamCMD update check for app ' + appId + '...');
  addLog(id, 'info', '🔄 This may take a few minutes — all output shown below');
  ...
  const handleLine = (line) => {
    line = line.trim();
    if (!line) return;
    const buildMatch = line.match(/Build ID[:\s]+(\d{6,})/i);
    if (buildMatch) newBuildId = buildMatch[1];

    if (line.includes('already up to date') || line.includes('up-to-date')) {
      const bid = newBuildId || currentBuildId;
      addLog(id, 'save', '✓ ' + line + (bid ? ' (build ' + bid + ')' : ''));
    } else if (line.includes('Success') && (line.includes('Update') || line.includes('update'))) {
      addLog(id, 'save', '✓ ' + line + (newBuildId ? ' — new build: ' + newBuildId : ''));
      updated = true;
      if (newBuildId) instances[id].buildId = newBuildId;
    } else if (line.includes('Error') || line.includes('error') || line.includes('FAILED')) {
      addLog(id, 'warn', '[SteamCMD] ' + line);
    } else {
      addLog(id, 'info', '[SteamCMD] ' + line);
    }
  };
  ...
  updateProc.on('exit', (code) => {
    instances[id].updating = false;
    const postBuildId = getInstalledBuildId(cfg.serverPath);
    if (postBuildId) instances[id].buildId = postBuildId;

    if (code === 0) {
      if (updated) {
        addLog(id, 'save', postBuildId ? '✓ Updated to build ' + postBuildId + ' — starting server...' : '✓ Update applied — starting server...');
      } else {
        addLog(id, 'info', postBuildId ? '✓ Already on latest build ' + postBuildId + ' — starting server...' : '✓ Server already up to date — starting...');
      }
    } else {
      addLog(id, 'warn', '⚠ SteamCMD exited with code ' + code + ' — starting server anyway...');
    }
    callback(updated);
  });
      ...
      const poll = setInterval(() => {
        ...
        if (!getInstance(id).process || waited > 45000) {
          ...
          if (!getInstance(id).process) {
            ...
            const skipUpdate = !(getServer(id)?.restartUpdateCheck !== false);
            startServer(id, skipUpdate);
          } else {
            addLog(id, 'warn', `⏰ ${reason} — server did not stop in time, skipping restart`);
          }
          // Schedule next restart AFTER server start attempt, not before
          initScheduler(id);
        }
      }, 1000);
    } else {
